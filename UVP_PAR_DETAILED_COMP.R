...
detailed_data <- readr::read_tsv(
  "C:/Users/aarfer/OneDrive - NOC/Documents/BIOCARBON+/COMPARISON/UVP5_DETAILED_TSVS/export_detailed_20260119_10_07_PAR_CAL2rcf19.tsv",
...
# -------------------------------
binning_scheme <- "uvp5"   # options: "global_log", "uvp5"
...
# --- uvp5 bin edges, adding open-ended last bin ---
uvp5_bin_edges_um <- c(uvp_bins$bin_min, 26000)
...
} else if (binning_scheme == "uvp5") {
  bin_edges_um <- uvp5_bin_edges_um
} else {
...
#######################################################
#constants
pxl <- 94 # this is the pixel size in the export summary for UVP5 73 is UVP6
...
# -------------------------------
par <- readr::read_tsv(
  "C:/Users/aarfer/OneDrive - NOC/Documents/BIOCARBON+/COMPARISON/UVP5_RAW_TSVS/20240621231619_CAL2rcf19_PAR_raw_20260119_07_57.tsv",
  locale = locale(encoding = "windows-1252"))

Aa  <- 0.0051    # check your instrument's UVPdb/calibration record
Exp <- 1.0645   # check your instrument's UVPdb/calibration record

######
#####
####
##### this is the section of importance for image correction ####################################################
particles_par <- par %>%
  mutate(
...
# -------------------------------
# 2. Depth bins (from UVP5 detailed export)
# -------------------------------
...
bin_thickness <- median(diff(depth_bins$depth_mid))
...
# -------------------------------
# 6. Load UVP5 official detailed PAR export
# -------------------------------
...
cat("Total particles in UVP5 official PAR:", total_particles, "\n")
...
p_integrated <- ggplot() +
  ...
  geom_point(data = uvp_official_integrated, aes(bin_mid, count_norm, color = "UVP5 detailed"), size = 2.5, shape = 17) +
  geom_line(data = uvp_official_integrated, aes(bin_mid, count_norm, color = "UVP5 detailed"), alpha = 0.5, linetype = "dashed") +
  scale_x_log10() +
  scale_y_log10()+
  scale_color_manual(values = c("PAR raw" = "blue", "UVP5 detailed" = "red")) +
  theme_custom() +
  labs(
    x = "ESD (µm)",
    y = expression(N / (Delta*D * V[total])),
    title = "Vertically Integrated UVP5 Size Spectra Comparison",
    color = "Source") +

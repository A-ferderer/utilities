...
process_uvp_raw <- function(raw_file, detailed_file, pxl, instrument_name) {
  
  # --- Extract raw file "middle section" for profile_id ---
  raw_basename <- tools::file_path_sans_ext(basename(raw_file))
  # Extract the part between the first underscore after the date and before the next underscore
  ...
  uvp5_spectra_depth <- particles_binned %>%
    ...
    group_modify(~{
      ...
      tibble(
        ...
        profile_id = profile_id  # assign cleaned profile_id
      )
    }) %>% ungroup()
...
all_uvp5_spectra <- map2(uvp5_raw_files, uvp5_detailed_files, 
                         ~process_uvp_raw(.x, .y, pxl = 94, instrument_name = "uvp5"))
...
all_uvp6_spectra <- map2(uvp6_raw_files, uvp6_detailed_files,
                         ~process_uvp_raw(.x, .y, pxl = 73, instrument_name = "uvp6"))
...
for (p in plots) {
  ggsave(
    filename = paste0("figures/uncorrected/", p, ".png"),
    plot = get(p),
    width = 7,
    height = 9,
    dpi = 300
...
# --- save
#ggsave("figures/weeholo_2x2_uncorrected.png", plot = weeholo_2x2, width = 7, height = 6, dpi = 600)
...
# --- save
#ggsave("figures/lisstholo_2x3_uncorrected.png", plot = LisstHolo_comparison, width = 8, height = 10, dpi = 600)
...
p_cal2rcf19
# --- save
#ggsave("figures/uvp6_uncorrected.png", plot = p_cal2rcf19, width = 7, height = 5, dpi = 600)
...
# --- save CSV
#write_csv(all_integrated_export, "outputs/all_integrated_psd_clean_uncorrected.csv")
